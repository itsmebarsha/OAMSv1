...
function ViewSubmissionList() {
  ...
  async function fetchAssignments(studentEmail) {
    try {
      const response = await axios.get(
        `/api/student/viewSubmission/${studentEmail}/${schoolCode}`
      );
  ...
  useEffect(() => {
    ...
    if (users?.user?.studentEmail) {
      fetchAssignments(users.user.studentEmail);
    }
