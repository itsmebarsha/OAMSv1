...
function ViewSubmissionList() {
  ...
  async function fetchAssignments(teacherEmail) {
    try {
      const response = await axios.get(
        `/api/teacher/viewSubmission/${teacherEmail}/${schoolCode}`
      );
  ...
  useEffect(() => {
    ...
    if (users?.user?.teacherEmail) {
      fetchAssignments(users.user.teacherEmail);
    }
  ...
  const submitScore = async (item, index) => {
    if (scoreValues[index]) {
      ...
      try {
        ...
        const response = await axios.post("/api/teacher/evaluation", formData);
                              ...
                              {filteredData != null ? (
                                filteredData?.map((item, index) => (
                                  <tr key={index}>
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                      <a href={item.submissionFile} download>
                                        Download
                                      </a>
                                    </td>
