...
function Addassignment() {
  ...
  const fetchSubjectData = async () => {
    try {
      const response = await axios.get(
        `/api/getData/subjectDataTeacher/${schoolCode}/${schoolStore.userData.userEmail}`
      );
  ...
  const handleSubject = (selectedSubject) => {
    const secondWord = selectedSubject.split("-")[1];
    console.log(secondWord);

    const selectedSubjectData = dataAllSubject.find(
      (course) => course.subjectName === secondWord
    );
    console.log(selectedSubjectData);

    if (selectedSubjectData) {
      setSemester(selectedSubjectData.subjectSemester);
    } else {
      setSemester([]);
    }
  };
                ...
                <div className="col-md-12 form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      handleSubject(e.target.value); // Pass the selected subject to handleSubject
                    }}>
