...
function ViewAssignmentList() {
  ...
  async function fetchAssignments(assignedSubject) {
    try {
      const response = await axios.get(
        `/api/teacher/viewAssignment/${
          schoolStore.userData.userEmail
        }/${schoolCode}?subjects=${assignedSubject.join(",")}`
      );
  ...
  useEffect(() => {
    if (users?.user?.assignedSubject) {
      const assignedSubjects = users.user.assignedSubject;

      // Fetch assignments based on assigned subjects
      fetchAssignments(assignedSubjects);
    }
